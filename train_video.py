...
def main(args):
    ...
    config.dataset.kwargs.update(_type='valid')
    ...
    valid_dataloader = _get_instance(src.datasets, config.dataloader, valid_dataset)
    ...
    optimizer = _get_instance(torch.optim, config.optimizer, temp_war_net.parameters())
    ...
    config.logger.kwargs.update(log_dir=saved_dir / 'log', net=temp_war_net, dummy_input=dummy_input)
