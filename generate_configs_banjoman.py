...
def generate_neural_warping_yaml(data_dir, src_id, tgt_id, n_kpts):
    ...
    tgt_id: 001
    """
    pair_dir = osp.join(data_dir, f'{src_id}_{tgt_id}')
    out_dir = osp.join(data_dir, f'{src_id}_{tgt_id}', f'{n_kpts}')
    config = {
        'main': {
            'random_seed': '6666',
            'saved_dir': out_dir
        ...
        'dataset': {
            'name': 'WarpingDataset',
            'src_kpts_path': osp.join(pair_dir, f'{n_kpts}', f"vw_{src_id}_000.dat"),
            'tgt_kpts_path': osp.join(pair_dir, f'{n_kpts}', f"vw_{tgt_id}_000.dat"),
            'src_img_path': osp.join(data_dir, f'vw_{src_id}_000.png'),
            'tgt_img_path': osp.join(data_dir, f'vw_{tgt_id}_000.png'),
            'kwargs': {
                'time_range': [-1.0, 1.0],
                'n_samples': 20000
            }
        ...
        'im_net': {
            'src_ckpt_path': osp.join(data_dir, 'image_models', f'vw_{src_id}_000', 'checkpoints', 'model_best.pth'),
            'tgt_ckpt_path': osp.join(data_dir, 'image_models', f'vw_{tgt_id}_000', 'checkpoints', 'model_best.pth'),
            'name': 'SIREN',
            'kwargs': {
                'in_channels': 2,
                'out_channels': 3,
